# BuildingBlocks.Pagination v1.1 — test matrix
...
| `HasKeysetIndex_` | Composite index property order + DESC flags match `SortKey`; nested path → `UnsupportedKeysetIndex`; `HasKeysetIndex(sortKey, NullOrder)` emits non-default `NULLS FIRST/LAST` in Npgsql `CREATE INDEX`; one-arg overload does not |
...
| `Postgres_` / `Mixed_Direction_` / `SqlServer_` / `Sqlite_` / `Three_Column_` / `Four_Column_` / `Nine_Column_` | Dapper: Postgres tuple SQL when directions match (incl. 3–4 and 9 columns); mixed ASC/DESC is OR; `NULLS FIRST/LAST` on PG/Sqlite ORDER BY; SqlServer `FETCH NEXT` and no NULLS. EF Sqlite: OR-chain + multi-column execute (incl. 9-column). EF Npgsql (Testcontainers, required in `pagination.yml` CI): tuple seek for uniform non-null keys of any width; string/mixed stay OR; NULLS LAST via interceptor |
...
| `Nullable_String_` / `AddBuildingBlocksPagination_` | EF Sqlite with `UseBuildingBlocksPagination`: `NullOrder.Last`/`First` change first-page order; without interceptor, Sqlite NULL-first remains. Seek aligns with NULLS semantics |
...
| Lab HTTP | `FeatureFusionApiTests` — `GET /api/v1/products-page` first/next/prev; empty cursor + `pageDirection=Backward` last page; GET matches POST; every `ProductSortField` × direction (incl. `NameThenPrice`); Price next-page seek SQL is Npgsql row comparison; JSON `"SortBy":"Price"`; invalid cursor 400; first-page `TotalCount > limit`; EF list uses `AsNoTracking` + `TagWith("products.list")` + SQL `Select` to `ProductDto`; `POST /api/v1/Product/products` and GET Minimal API return the same ids; Dapper first page matches EF; Swagger v1 documents GET |
