...
| MCP write / agent-client semantics | 6, 13, 14, 16 | Confirm+replay, regenerated keys amplify, concurrent same-key safe, **IMcpRateLimiter** bounds distinct-key storms. Distributed wait-and-replay (`UseRedisLock`) + 2026 MRTR confirmation are **BuildingBlocks.Mcp 1.1.0** (Lab overlay `McpDistributedIdempotency` / `McpMrtrConfirmation`; default host stays memory). |
...
- Each packable project owns `<Version>` in its `.csproj` (e.g. `BuildingBlocks.Pagination.EntityFrameworkCore` `1.1.0`, `BuildingBlocks.Idempotency` `1.0.1`, `BuildingBlocks.Mcp` `1.1.0`).
